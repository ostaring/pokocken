...
import { useAdminRecipesQueryWithFilters } from "@/features/recipes/hooks/recipe-hooks";

const { mockFetchAdminRecipes } = vi.hoisted(() => ({
  mockFetchAdminRecipes: vi.fn(),
}));

vi.mock("@/lib/api/recipes/recipes", async () => {
  const actual = await vi.importActual<typeof import("@/lib/api/recipes/recipes")>(
    "@/lib/api/recipes/recipes",
  );
